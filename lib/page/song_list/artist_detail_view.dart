...
class ArtistDetailView extends StatelessWidget {
  ...
  const ArtistDetailView({super.key, required this.onBack});
...
class _ArtistHeaderState extends State<_ArtistHeader> {
  ...
  @override
  void didUpdateWidget(covariant _ArtistHeader oldWidget) {
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                _ArtistCoverArt(cover: widget.cover),
                ...
                Expanded(
                  child: AnimatedSwitcher(
                    ...
                    child: widget.isSearching
                        ? _HeaderSearchField(
                            key: const ValueKey('artist_header_search'),
                            foreground: foreground,
                            onChanged: widget.onSearchChanged,
                            onClose: widget.onCloseSearch,
                          )
                        : _HeaderArtistInfo(
                            key: const ValueKey('artist_header_info'),
                            title: widget.title,
...
class _HeaderArtistInfo extends StatelessWidget {
  final String title;
  final int songCount;
  final Color foreground;
  final Color subtleForeground;

  const _HeaderArtistInfo({
    super.key,
    required this.title,
    required this.songCount,
...
class _ArtistHeaderBackdrop extends StatelessWidget {
  final Uint8List? cover;
  final Color baseColor;

  const _ArtistHeaderBackdrop({required this.cover, required this.baseColor});
...
class _ArtistCoverArt extends StatelessWidget {
  final Uint8List? cover;

  const _ArtistCoverArt({required this.cover});
